...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const pointers = [new FluidPointer()];
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement): WebGLContextResult {
      ...
      if (!glCtx) throw new Error("WebGL is not supported in this browser");

      let halfFloat: any;
      let supportLinearFiltering: object | null;
      if (isWebGL2) {
    ...
    function updatePointerMoveData(pointer: FluidPointer, posX: number, posY: number, color: { r: number; g: number; b: number }) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
